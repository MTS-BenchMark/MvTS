...
class StemGNNExecutor(AbstractExecutor):
    ...
    def train(self, train_loader, valid_loader):
        ...
        for epoch in range(self.epochs):
            ...
            if (epoch + 1) % self.validate_freq == 0:
                ...
                performance_metrics, _ = self.validate(valid_data)
    ...
    def evaluate(self, test_dataloader):
        ...
        test_data = [test_dataloader, self.test_normalize_statistic]
        with torch.no_grad():
            self.model.eval()
            _, escore = self.validate(test_data)
    ...
    def validate(self, data): #验证
        with torch.no_grad():
            ...
            for i, (inputs, target) in enumerate(dataloader):
                inputs = inputs.to(self.device)
                target = target.to(self.device)
                output, a = self.model(inputs)
                preds.append(output.detach().cpu().numpy())
                truths.append(target.detach().cpu().numpy())
            preds = np.concatenate(preds, axis=0) #[num, 1, num_nodes]
