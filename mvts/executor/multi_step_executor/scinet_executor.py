...
class SCINetExecutor(AbstractExecutor):
    ...
    def train(self, train_data, valid_data):
        ...
        for epoch in tqdm.tqdm(range(1, self.epochs + 1)):
            ...
            for iter, (x,y) in enumerate(train_data.get_iterator()):
                ...
                trainx = torch.reshape(trainx, (self.batch_size, self.window, -1))
                trainy = torch.reshape(trainy, (self.batch_size, self.window, -1))
            ...
            for iter, (x, y) in enumerate(valid_data.get_iterator()):
                ...
                valx = torch.reshape(valx, (self.batch_size, self.window, -1))
                valy = torch.reshape(valy, (self.batch_size, self.window, -1))
                with torch.no_grad():
    ...
    def evaluate(self, test_data):
        ...
        for iter, (x, y) in enumerate(test_data.get_iterator()):
            ...
            testx = torch.reshape(testx, (self.batch_size, self.window, -1))
            testy = torch.reshape(testy, (self.batch_size, self.window, -1))
            with torch.no_grad():
