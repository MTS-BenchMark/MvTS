...
class CCRNNExecutor(MultiStepExecutor):
    ...
    def train(self, train_data, valid_data):
        ...
        for epoch in range(1, self.epochs + 1):
            ...
            for iter, (x,y) in enumerate(train_data.get_iterator()):
                ...
                output, graph = self.model(trainx, trainy, self.train_batch_seen)
            ...
            for iter, (x, y) in enumerate(valid_data.get_iterator()):
                self.model.eval()
                valx = torch.Tensor(x).to(self.device)  # [batch_size, window, num_nodes, dim]
                valy = torch.Tensor(y).to(self.device)  # [batch_size, horizon, num_nodes, dim]
                # valx, valy = self._prepare_data(x, y) 
                with torch.no_grad():
                    output, _ = self.model(valx, None, self.train_batch_seen)
    ...
    def evaluate(self, test_data):
        ...
        for iter, (x, y) in enumerate(test_data.get_iterator()):
            ...
            with torch.no_grad():
                ...
                pred, _ = self.model(testx, None, self.train_batch_seen)
                outputs.append(pred) 
                realy.append(testy)
