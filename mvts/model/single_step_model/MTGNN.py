...
class MTGNN(nn.Module):
    def __init__(self, config, data_feature):
        ...
        self.scale = self.data_feature['scale']
        self.rse = self.data_feature["rse"]
        self.rae = self.data_feature["rae"]
        self.gcn_true = self.config.get('gcn_true', True)
        self.buildA_true = self.config.get('buildA_true', True)
        self.gcn_depth = self.config.get('gcn_depth', 2)
        self.num_nodes = self.config.get('num_nodes', 40)
        self.device = torch.device(self.config.get('device', "cuda:0"))
        self.static_feat = self.data_feature.get('static_feat', None)
        self.subgraph_size = self.config.get('subgraph_size', 20)
        if self.subgraph_size > self.num_nodes:
            self.subgraph_size = self.num_nodes
        self.node_dim = self.config.get('node_dim', 40)
        self.dropout = self.config.get('dropout', 0.3)
        self.predefined_A = self.data_feature.get('predefined_A', None)
        self.dilation_exponential = self.config.get('dilation_exponential', 1)
        self.conv_channels = self.config.get('conf_channels', 32)
        self.residual_channels = self.config.get('residual_channels', 32)
        self.skip_channels = self.config.get('skip_channels', 64)
        self.end_channels = self.config.get('end_channels', 128)
        self.seq_length = self.config.get('seq_in_len', 168)
        self.in_dim = self.config.get('in_dim', 2)
        self.out_dim = self.config.get('seq_out_len', 1)
        self.layers = self.config.get('layers', 3)
        self.propalpha = self.config.get('propalpha', 0.05)
        self.tanhalpha = self.config.get('tanhalpha', 3)
        self.layer_norm_affline = self.config.get('layer_norm_affline', False)
        ...
        self.start_conv = nn.Conv2d(in_channels=self.config.get('in_dim', 1),
                                    out_channels=self.config.get('residual_channels', 16),
                                    kernel_size=(1, 1))
