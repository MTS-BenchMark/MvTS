...
class StemGNN(nn.Module):
    def __init__(self, config, data_feature):
        ...
        self.train_scaler = self.data_feature['train_scaler']
        self.valid_scaler = self.data_feature['valid_scaler']
        self.test_scaler = self.data_feature['test_scaler']

        self.config = config
        self.device = torch.device(self.config.get('device', "cpu"))
        self.stack_cnt = self.config.get("stack_cnt", 2) #StemGNN Block的数量
        self.unit = self.config.get("num_nodes",140)
        self.alpha = self.config.get("leakyrelu_rate", 0.2)
        self.dropout_rate = self.config.get("dropout_rate", 0.5)
        self.time_step = self.config.get("window_size", 12)
        self.horizon = self.config.get("horizon", 3)
        self.weight_key = nn.Parameter(torch.zeros(size=(self.unit, 1)).to(self.device))
        nn.init.xavier_uniform_(self.weight_key.data, gain=1.414)
        self.weight_query = nn.Parameter(torch.zeros(size=(self.unit, 1)).to(self.device))
        nn.init.xavier_uniform_(self.weight_query.data, gain=1.414)
        self.GRU = nn.GRU(self.time_step, self.unit)
        self.multi_layer = self.config.get("multi_layer", 5)
        ...
        self.fc = nn.Sequential(
            ...
            nn.LeakyReLU(),
            nn.Linear(int(self.time_step), 1), #TODO
        )
    ...
    def get_laplacian(self, graph, normalize):
        ...
        """
        if normalize:
            D = torch.diag(torch.sum(graph, dim=-1) ** (-1 / 2)).to(self.device)
            L = torch.eye(graph.size(0), device=graph.device, dtype=graph.dtype) - torch.mm(torch.mm(D, graph), D)
        else:
            D = torch.diag(torch.sum(graph, dim=-1)).to(self.device)
            L = D - graph
        return L
    ...
    def forward(self, x):
        ...
        forecast = self.fc(forecast) #[batch, num_nodes, output_seq_len]
