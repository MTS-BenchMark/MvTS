...
class Encoder(nn.Module):
    ...
    def __init__(self, input_size: int, hidden_size: int, T: int):
        ...
        # self.attn_linear = nn.Sequential(nn.Linear(2 * hidden_size + T,
        #                                          hidden_size),
        #                                nn.Tanh(),
    ...
    def forward(self, input_data):
        ...
        input_encoded = Variable(torch.zeros(input_data.size(0), self.T, self.hidden_size).to(device))
...
class DARNN(nn.Module):
    def __init__(self, config, data_feature):
        ...
        global device
        device = torch.device(self.config.get('device', ""))

        self.input_dim = self.config.get("input_dim", 1)
        self.num_nodes = self.config.get("num_nodes", 1)
        self.output_dim = self.config.get("output_dim", 1)
        self.encoder_hidden_size = self.config.get("encoder_hidden_size", 64)
        self.decoder_hidden_size = self.config.get("decoder_hidden_size", 64)
        self.window = self.config.get("window", 168)

        
        self.encoder = Encoder(self.num_nodes * self.input_dim, 
                            self.encoder_hidden_size, 
                            self.window)

        self.decoder = Decoder(self.encoder_hidden_size, 
    ...
    def forward(self, x):
        # input x : (batch_size, window_size, num_nodes * input_dim)
        # outoput y: (batch_size, num_nodes*output_dim)

        assert len(x.shape) == 3

        input_weighted, input_encoded = self.encoder(x)
        
        y_pred = self.decoder(input_encoded, x)
