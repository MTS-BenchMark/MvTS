...
class STEmbedding(nn.Module):
    ...
    def forward(self, SE, TE, T=288):
        ...
        for i in range(TE.shape[0]):
            dayofweek[i] = F.one_hot(TE[..., 0][i].to(torch.int64) % 7, 7).to(self.device)
        for j in range(TE.shape[0]):
            timeofday[j] = F.one_hot(TE[..., 1][j].to(torch.int64) % T, T).to(self.device)
...
class GMAN(nn.Module):
    def __init__(self, config, data_feature):
        ...
        self.bn_decay = self.config.get('bn_decay')
        self.num_his = self.config.get('window_size')
    ...
    def forward(self, X, TE):
        # input
        # print('X.shape: ', X.shape) #[batch_size, seq_length, nodes_num]
        ...
        # exit()
        return torch.squeeze(X, 3)  # [batch_size, seq_length, num_nodes]