...
class AutoFormer(nn.Module):
    ...
    def __init__(self, config, data_feature):
        ...
        self.seq_len = self.config.get("window", 12)
        self.label_len = self.config.get("label_len", 12)
        self.pred_len = self.config.get("horizon", 12)
        self.output_attention = self.config.get("output_attention", False)

        # Decomp
        kernel_size = self.config.get("moving_avg", 25)

        self.decomp = series_decomp(kernel_size)


        num_nodes = self.config.get("num_nodes", 0)
        input_dim = self.config.get("input_dim", 1)
        output_dim = self.config.get("output_dim", 1)

        dim = min(input_dim, output_dim)

        enc_in = num_nodes * dim
        dec_in = num_nodes * dim
        c_out = num_nodes * dim

        d_model = self.config.get("d_model", 512)

        embed = self.config.get("embed")
        freq = self.config.get("freq")
        dropout = self.config.get("dropout", 0.05)
        factor = self.config.get("attn_factor", 1)

        n_heads = self.config.get("n_heads")
        d_ff = self.config.get("d_ff")
