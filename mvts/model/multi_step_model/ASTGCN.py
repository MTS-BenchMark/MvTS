...
class ASTGCN(nn.Module):
    def __init__(self, config, data_feature):
        ...
        self.encoder_input_size = self.config.get('encoder_input_size')
        self.decoder_input_size = self.config.get('decoder_input_size')
        self.num_of_vertices = self.config.get('num_of_vertices')
        self.num_of_weeks = self.config.get('num_of_weeks')
        self.num_of_days = self.config.get('num_of_days')
        self.num_of_hours = self.config.get('num_of_hours')
        self.num_layers = self.config.get('num_layers')
        self.points_per_hour = self.config.get('points_per_hour')
        self.num_for_predict = self.config.get('horizon')
        self.d_model = self.config.get('d_model')
        self.nb_head = self.config.get('nb_head')
        self.ScaledSAt = self.config.get('ScaledSAt')  # whether use spatial self attention
        self.SE = self.config.get('SE')  # whether use spatial embedding
        self.smooth_layer_num = self.config.get('smooth_layer_num')
        self.aware_temporal_context = self.config.get('aware_temporal_context')
        ...
        norm_Adj_matrix = torch.from_numpy(norm_Adj(self.adj_mx)).type(torch.FloatTensor).to(self.device)  # 通过邻接矩阵，构造归一化的拉普拉斯矩阵
        num_of_vertices = norm_Adj_matrix.shape[0]

        src_dense = nn.Linear(self.encoder_input_size, self.d_model)

        if self.ScaledSAt:  # employ spatial self attention
            position_wise_gcn = PositionWiseGCNFeedForward(spatialAttentionScaledGCN(norm_Adj_matrix, self.d_model, self.d_model),
                                                           dropout=self.dropout)
        else:  # 不带attention
            position_wise_gcn = PositionWiseGCNFeedForward(spatialGCN(norm_Adj_matrix, self.d_model, self.d_model),
                                                           dropout=self.dropout)

        trg_dense = nn.Linear(self.decoder_input_size, self.d_model)  # target input projection
        ...
        # print('self.num_of_weeks: ', self.num_of_weeks)
        # print('self.num_for_predict: ', self.num_for_predict)
        # print('self.num_of_days: ', self.num_of_days)
        # print('self.num_of_hours: ', self.num_of_hours)
        max_len = max(self.num_of_weeks * 7 * 24 * self.num_for_predict, self.num_of_days * 24 * self.num_for_predict,
                      self.num_of_hours * self.num_for_predict)

        w_index = search_index(max_len, self.num_of_weeks, self.num_for_predict, self.points_per_hour, 7 * 24)
        d_index = search_index(max_len, self.num_of_days, self.num_for_predict, self.points_per_hour, 24)
        h_index = search_index(max_len, self.num_of_hours, self.num_for_predict, self.points_per_hour, 1)
        ...
        if self.aware_temporal_context:  # employ temporal trend-aware attention
            attn_ss = MultiHeadAttentionAwareTemporalContex_q1d_k1d(self.nb_head, self.d_model, self.num_of_weeks, self.num_of_days,
                                                                    self.num_of_hours, self.num_for_predict, self.kernel_size,
                                                                    dropout=self.dropout)  # encoder的trend-aware attention用一维卷积
            attn_st = MultiHeadAttentionAwareTemporalContex_qc_k1d(self.nb_head, self.d_model, self.num_of_weeks, self.num_of_days,
                                                                   self.num_of_hours, self.num_for_predict, self.kernel_size,
                                                                   dropout=self.dropout)
            att_tt = MultiHeadAttentionAwareTemporalContex_qc_kc(self.nb_head, self.d_model, self.num_of_weeks, self.num_of_days,
                                                                 self.num_of_hours, self.num_for_predict, self.kernel_size,
                                                                 dropout=self.dropout)  # decoder的trend-aware attention用因果卷积
        else:  # employ traditional self attention
            attn_ss = MultiHeadAttention(self.nb_head, self.d_model, dropout=self.dropout)
            attn_st = MultiHeadAttention(self.nb_head, self.d_model, dropout=self.dropout)
            att_tt = MultiHeadAttention(self.nb_head, self.d_model, dropout=self.dropout)

        if self.SE and self.TE:
            encode_temporal_position = TemporalPositionalEncoding(self.d_model, self.dropout, max_len,
                                                                  en_lookup_index)  # decoder temporal position embedding
            decode_temporal_position = TemporalPositionalEncoding(self.d_model, self.dropout, self.num_for_predict)
            spatial_position = SpatialPositionalEncoding(self.d_model, num_of_vertices, self.dropout,
                                                         GCN(norm_Adj_matrix, self.d_model, self.d_model),
                                                         smooth_layer_num=self.smooth_layer_num)
        ...
        encoderLayer = EncoderLayer(self.d_model, attn_ss, c(position_wise_gcn), self.dropout,
                                    residual_connection=self.residual_connection, use_LayerNorm=self.use_LayerNorm)
        encoder = Encoder(encoderLayer, self.num_layers)

        decoderLayer = DecoderLayer(self.d_model, att_tt, attn_st, c(position_wise_gcn), self.dropout,
                                    residual_connection=self.residual_connection, use_LayerNorm=self.use_LayerNorm)
        decoder = Decoder(decoderLayer, self.num_layers)

        generator = nn.Linear(self.d_model, self.decoder_input_size)

        # print('-------begin print model parameters()------')
        ...
        for my_model in [self.encoder, self.decoder, self.src_embed, self.trg_embed, self.generator]:
    ...
    def forward(self, src, trg):
        ...
        '''
        encoder_output = self.encode(src)  # (batch_size, N, T_in, d_model)
        decoder_output = self.decode(trg, encoder_output)
    ...
    def decode(self, trg, encoder_output):
        ...
        return self.generator(self.decoder(h, encoder_output))
