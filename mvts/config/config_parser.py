...
class ConfigParser(object):
    ...
    def _load_default_config(self):
        # 首先加载 task config
        with open('./mvts/config/task_config.json', 'r') as f:
        ...
        # dataset
        default_file_list.append('data/{}.json'.format(self.config['dataset']))
        # 加载所有默认配置
        for file_name in default_file_list:
            with open('./mvts/config/{}'.format(file_name), 'r') as f:
                x = json.load(f)
                for key in x:
                    if key not in self.config:
                        self.config[key] = x[key]
