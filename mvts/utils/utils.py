...
def get_executor(config, model):
    ...
    """
    if config["task"] == "single_step":
        try:
            return getattr(importlib.import_module('mvts.executor.single_step_executor'),
                        config['executor'])(config, model)
        except AttributeError:
            raise AttributeError('executor is not found')
    elif config["task"] == "multi_step":
        try:
            return getattr(importlib.import_module('mvts.executor.multi_step_executor'),
                        config['executor'])(config, model)
        except AttributeError:
            raise AttributeError('executor is not found')
    else:
...
def get_model(config, data_feature):
    ...
    if config['task'] == 'multi_step':
        ...
        try:
            return getattr(importlib.import_module('mvts.model.multi_step_model'),
        ...
        print("config[model]: ", config['model'])
        try:
            return getattr(importlib.import_module('mvts.model.single_step_model'),
                           config['model'])(config, data_feature)
        except AttributeError:
            raise AttributeError('model is not found')
...
def get_evaluator(config):
    ...
    """
    try:
        return getattr(importlib.import_module('mvts.evaluator'),
