...
class AGCRN(nn.Module):
    def __init__(self, config, data_feature):
        ...
        self.num_node = self.config.get("num_nodes", None)
        self.input_dim = self.config.get("input_dim", 1)
        self.hidden_dim = self.config.get("rnn_units", None)
        self.output_dim = self.config.get("output_dim", 1)
        self.horizon = self.config.get("horizon", 12)
        self.num_layers = self.config.get("num_layers", 12)
        self.embed_dim = self.config.get("embed_dim", None)
        self.cheb_k = self.config.get("cheb_order", None)
        self.rnn_units = self.config.get("rnn_units", None)

        self.default_graph = self.config.get("default_graph", True)
        self.node_embeddings = nn.Parameter(torch.randn(self.num_node, self.embed_dim), requires_grad=True)

        self.encoder = AVWDCRNN(self.num_node, self.input_dim, self.rnn_units, self.cheb_k,
