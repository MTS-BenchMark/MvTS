...
class SCINet(nn.Module):
    ...
    def __init__(self, config, data_feature):
        ...
        self.input_dim = self.config.get("num_nodes", 0)
        self.input_len = self.config.get("window", 12)
        self.output_len = self.config.get("horizon", 12)
        self.hidden_size = self.config.get("hidden-size", 8)
        self.num_levels = self.config.get("levels", 3)
        self.groups = self.config.get("groups", 1)
        self.modified = True
        self.kernel_size = self.config.get("kernel", 5)
        self.dropout = self.config.get("dropout", 0.5)
        self.single_step_output_One = self.config.get("single_step_output_One", 0)
        self.concat_len = self.config.get("concat_len", 0)
        self.pe = self.config.get("positionalEncoding", True)
        self.RIN=self.config.get("RIN", False)
        self.num_stacks = self.config.get("num_stacks", 1)
        ...
        if self.num_stacks == 2: # we only implement two stacks at most.
