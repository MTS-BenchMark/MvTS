...
class StemGNNExecutor(AbstractExecutor):
    def __init__(self, config, model):
        ...
        self.train_normalize_statistic = self.model.train_normalize_statistic
        self.valid_normalize_statistic = self.model.valid_normalize_statistic
        self.test_normalize_statistic = self.model.test_normalize_statistic
    ...
    def train(self, train_loader, valid_loader):
        ...
        for epoch in range(self.epochs):
            ...
            # early stop
            if self.early_stop and validate_score_non_decrease_count >= self.patience:
                print("early_stop at epoch:", epoch)
                self.model = self.best_model
                break

    def evaluate(self, test_data):
        dataloader, statistic = test_data, self.test_normalize_statistic
        forecast_norm, target_norm = self.inference(dataloader)
        if (self.normalize_method is not 0) and statistic:
            forecast = self.de_normalized(forecast_norm, statistic)
            target = self.de_normalized(target_norm, statistic)
        else:
        ...
        with torch.no_grad():
            for i, (inputs, target) in enumerate(dataloader):
                ...
                while step < self.horizon:
                    ...
                    forecast_steps[:, step:min(self.horizon - step, len_model_output) + step, :] = \
                        forecast_result[:, :min(self.horizon - step, len_model_output), :].detach().cpu().numpy()
    ...
    def validate(self, data): #验证
        dataloader, statistic = data[0], data[1]
        forecast_norm, target_norm = self.inference(dataloader)
        if (self.normalize_method is not 0) and statistic:
            forecast = self.de_normalized(forecast_norm, statistic)
            target = self.de_normalized(target_norm, statistic)
        ...
        print(f'MAPE {mape:7.9}; MAE {mae:7.9f}; RMSE {rmse:7.9f}.')
